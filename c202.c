...
void Stack_Init(Stack *stack) {
    if (stack == NULL) {
        // Call the Stack_Error function if the stack pointer is NULL
        Stack_Error(SERR_INIT);
        return;
    }
    
    stack->topIndex = -1; // Initialize the top index to indicate an empty stack
    stack->array = malloc(STACK_SIZE * sizeof(char)); // Allocate memory for the stack array
}
...
bool Stack_IsFull(const Stack *stack) {
    // Return true if the top index is at the maximum index (STACK_SIZE - 1), indicating a full stack
...
void Stack_Top(const Stack *stack, char *dataPtr) {
    if (Stack_IsEmpty(stack)) {
        // If the stack is empty, call the Stack_Error(SERR_TOP) function
        Stack_Error(SERR_TOP);
        return;
    }
    
    // Retrieve the character from the top of the stack and store it in 'dataPtr'
...
void Stack_Pop(Stack *stack) {
    if (Stack_IsEmpty(stack)) {
        // Handle popping from an empty stack (no action required for simplicity)
        return;
    }
    
    // Move the top index down to "pop" the element from the stack
    stack->topIndex--;
}
...
void Stack_Push(Stack *stack, char data) {
    if (Stack_IsFull(stack)) {
        // Handle pushing to a full stack (call Stack_Error for error handling)
...
void Stack_Dispose(Stack *stack) {
    // Reset the stack to an empty state by freeing the array and setting the top index to -1
    free(stack->array);
    stack->array = NULL; // Set the array pointer to NULL to avoid accessing it after disposal
    stack->topIndex = -1;
}
